...
const model = require("../models/itinerary_list");
...
exports.getOne = async (req, res, next) => {
	try {
		...
		const data = await model.findOne(itinerary_id);
		if (!_.isEmpty(data)) {
...
exports.update = async (req, res, next) => {
	try {
		...
		const data = await model.update(itinerary_id, req.body);
		if (!_.isEmpty(data)) {
			res.status(StatusCodes.OK).send(data[0]);
		} else {
			res.status(StatusCodes.BAD_REQUEST).send({message : "Bad request."});
...
exports.remove = async (req, res, next) => {
	try {
		...
		//const id = req.params.id;
		const data = await model.remove(itinerary_id);
		if (data) {
